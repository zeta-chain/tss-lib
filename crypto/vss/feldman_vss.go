...
//
func Create(curve elliptic.Curve,threshold int, secret *big.Int, indexes []*big.Int) (Vs, Shares, error) {
	...
	v := make(Vs, len(poly))
	for i, ai := range poly {
		v[i] = crypto.ScalarBaseMult(curve, ai)
	}
	...
	for i := 0; i < num; i++ {
		...
		share := evaluatePolynomial(curve,threshold, poly, indexes[i])
		shares[i] = &Share{Threshold: threshold, ID: indexes[i], Share: share,Curve: curve}
	}
	return v, shares, nil
}
...
func (share *Share) Verify(threshold int, vs Vs) bool {
	...
	var err error
	modQ := common.ModInt(share.Curve.Params().N)
	v, t := vs[0], one // YRO : we need to have our accumulator outside of the loop
	for j := 1; j <= threshold; j++ {
		// t = k_i^j
		t = modQ.Mul(t, share.ID)
		// v = v * v_j^t
		vjt := vs[j].SetCurve(share.Curve).ScalarMult(t)
		v, err = v.SetCurve(share.Curve).Add(vjt)
		if err != nil {
			return false
		}
	}
	sigmaGi := crypto.ScalarBaseMult(share.Curve, share.Share)
...
func (shares Shares) ReConstruct() (secret *big.Int, err error) {
	...
	modN := common.ModInt(shares[0].Curve.Params().N)
...
func samplePolynomial(curve elliptic.Curve,threshold int, secret *big.Int) []*big.Int {
	q := curve.Params().N
...
//
func evaluatePolynomial(curve elliptic.Curve,threshold int, v []*big.Int, id *big.Int) (result *big.Int) {
	q := curve.Params().N
